...
void RTCVars::debugOutputRTCVars() {
  byte *vb;
  char *vc;
  int *vi;
  long *vl;
  float *vf;
  Serial.println(F("debug output of all RTC variables:"));
  for (int i = 0; i < _state_variables_counter; i++) {
    Serial.print(F("Variable "));
    Serial.print(i);
    switch (_state_variables_type[i]) {
      case RTC_STATE_TYPE_CHAR:
        vc = reinterpret_cast<char*>(_state_variables_ptr[i]);
      ...
      case RTC_STATE_TYPE_INT:
        ...
        Serial.println(*vi);
        break;
      case RTC_STATE_TYPE_LONG:
        vl = reinterpret_cast<long*>(_state_variables_ptr[i]);
        Serial.print(F(" is type long: "));
        Serial.println(*vl);
        break;
      case RTC_STATE_TYPE_FLOAT:
        vf = reinterpret_cast<float*>(_state_variables_ptr[i]);
        Serial.print(F(" is type float: "));
        Serial.println(*vf);
        break;
    }
  }
  Serial.print(F("Overall size of RTC variables: "));
  Serial.println(_state_size);
  Serial.print(F("Free Memory in RTC: "));
  Serial.print(getFreeRTCMem());
  Serial.println(F(" bytes"));
  Serial.print(F("Free number of variables in RTC: "));
  Serial.print(getFreeRTCVars());
  Serial.println(F(" vars - adjust by #define RTC_MAX_VARIABLES x"));
}
...
bool RTCVars::_checkValidRTCData() {
  ...
  if (buf_head[0] != 'M') return false;
  if (buf_head[1] != 'G') return false;
  if (buf_head[2] != 'C') return false;

  _last_read_status = RTC_ERROR_SIZE;
  // check for valid size
  int size_in_rtc = (int)(buf_head[4] * 256 + buf_head[5]);
  if (size_in_rtc > RTC_MAX_SIZE) return false;
  ...
  for (int j = 0; j < size_in_rtc; j++) temp += buf[j]; //checksum
  if (temp != buf[size_in_rtc]) return false;
